        ...
        <div className="overflow-y-scroll h-[530px] ">
          {popularDishes.map((dish) => {
            return (
              <div
                key={dish.id}
                className="flex items-center gap-4 bg-[#1f1f1f] rounded-[15px] px-6 py-3 mt-4 mx-6"
              >
                <h1 className="text-[#f5f5f5] font-bold text-lg mr-5">{dish.id<10 ? `0${dish.id}` : dish.id}</h1>
                <img
                  src={dish.image}
                  alt={dish.name}
